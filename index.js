...
// --- Event Story Endpoints (Dynamic File Reading) ---

// 1. Get Event Story Index
app.get("/api/eventstory/index.json", (_req, res) => {
  res.json(eventStoryIndex);
});

// 2. Get Event Story Script Details
app.get("/api/eventstory/stories/:id.json", (req, res) => {
  const { id } = req.params;
  const safeId = id.replace(/[^a-zA-Z0-9_]/g, ""); // Allow underscores for adv_event_...
  const filePath = path.join(
    process.cwd(),
    "src/data/eventstory/detail/",
    `${safeId}.json`,
  );
...
const getGachaCategory = (gacha) => {
  ...
  const firstCardId =
    gacha.pickupCardIds && gacha.pickupCardIds.length > 0
      ? gacha.pickupCardIds[0].toLowerCase()
      : "";

  const standardPollLength =
    gacha.pickupCardIds && gacha.pickupCardIds.length === 1;
  ...
  if (
    name.includes("kizuna") ||
    name.includes("絆") ||
    firstCardId.includes("link")
  )
    return "Kizuna";
  ...
  // 5. RERUN / REVIVAL
  if (name.includes("rerun") || name.includes("復刻") || id.includes("rev"))
    return "Rerun";
  ...
  // 7. STANDARD / DIAMOND
  if (name.includes("★5アイドル\n1人確定ガチャ") && standardPollLength)
    return "Standard";
  if (
    name.includes("normal") ||
    name.includes("ダイヤガチャ") ||
    id.includes("normal")
  )
    return "Diamond";

  // Default fallback
  if (name.includes("pick up") || name.includes("ピックアップ"))
    return "Rate Up";
};
...
app.get("/api/gachas", (_req, res) => {
  const list = gachaList
    .filter((g) => {
      ...
      const firstCardId =
        g.pickupCardIds && g.pickupCardIds.length > 0
          ? g.pickupCardIds[0].toLowerCase()
          : "";

      // Filter Sampah
      if (g.name.includes("パック") || g.name.includes("Pack")) return false;
      if (g.name.includes("Ticket") || g.name.includes("チケット"))
        return false;
...
app.get("/api/gachas/:id/pool", (req, res) => {
  ...
  const cleanPickupIds = (banner.pickupCardIds || []).map((pid) =>
    pid.replace(/^card-/, ""),
  );

  const rateUpCards = allCards.filter(
    (c) =>
      banner.pickupCardIds?.includes(c.uniqueId) ||
      cleanPickupIds.includes(c.uniqueId),
  );
  ...
  const standardPool = allCards.filter((c) => {
    ...
    const isFes =
      cardCat.includes("fest") ||
      cardCat.includes("fes") ||
      uid.includes("fes");
    const isKizuna =
      cardCat.includes("link") ||
      cardCat.includes("kizuna") ||
      uid.includes("link");
    const isBirthday = cardCat.includes("birthday") || uid.includes("birt");
    const isLimited =
      (cardCat.includes("limited") || uid.includes("lm-")) &&
      !isFes &&
      !isKizuna &&
      !isBirthday;
...
app.get("/api/music/songs", (req, res) => {
  ...
  if (!fs.existsSync(filePath)) {
    return res
      .status(404)
      .json({ error: "Data not found. Run sync script first." });
  }
  ...
  try {
    const response = await fetch(url.toString(), {
      headers: {
        ...
        Origin: "https://polaris.dasewasia.my.id",
      },
    });
