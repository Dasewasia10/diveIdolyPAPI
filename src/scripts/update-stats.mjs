...
async function main() {
  console.log(
    "🚀 Memulai Update Stats & Data Teknis (Tanpa Menimpa Terjemahan)...",
  );
  ...
  try {
    ...
    console.warn(
      "⚠️ Tidak ada data lama, semua akan dianggap baru (Untranslated).",
    );
  }
  ...
  const updateSkill = (skillId, oldSkillData) => {
    ...
    let iconUrl = "";
    if (typeSkill === "SP")
      iconUrl = `${R2_DOMAIN}/iconSkillYell/img_icon_skill_${sData.assetId}.png`;
    else {
      let suffix = "unknown";
    ...
    return {
      typeSkill,
      name: oldSkillData?.name || {
        japanese: sData.name,
        global: sData.name,
        indo: sData.name,
      },
      description: oldSkillData?.description || {
        japanese: [lData.description],
        global: [lData.description],
        indo: [lData.description],
      },

      // UPDATE DATA TEKNIS (Selalu pakai yang baru dari GitHub)
      ct: lData.coolTime,
      staminaUsed: lData.stamina,
      probability: lData.probabilityPermil
        ? lData.probabilityPermil / 10
        : null,
  ...
  const updateYell = (lId, aId, oldYellData) => {
    ...
    return {
      name: oldYellData?.name || {
        japanese: aData.name,
        global: aData.name,
        indo: aData.name,
      },
      description: oldYellData?.description || {
        japanese: descRaw,
        global: descRaw,
        indo: descRaw,
      },
      source: {
        initialImage: `${R2_DOMAIN}/iconSkillYell/img_icon_yell-${imageId}.png`,
      },
    };
  };
  ...
  for (const card of cardsRaw) {
    ...
    groupedData[charName].data.push({
      ...
      description: cachedCard?.description || {
        ...
        indo: [card.description],
      },
      obtainMessage: cachedCard?.obtainMessage || {
        japanese: card.obtainMessage,
        global: card.obtainMessage,
        indo: card.obtainMessage,
      },

      // --- DATA TEKNIS (Always Update) ---
      releaseDate: !isNaN(new Date(parseInt(card.releaseDate)))
        ? new Date(parseInt(card.releaseDate)).toISOString().split("T")[0]
        : "1970-01-01",
      category: category,
      costumeTheme,
      costumeIndex: 0,
  ...
  console.log(
    `✅ Update Selesai! Stats & Data Teknis diperbarui di ${OUTPUT_PATH}`,
  );
}
