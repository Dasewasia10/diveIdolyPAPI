...
// --- KONFIGURASI ---
const R2_DOMAIN = "https://apiip.dasewasia.my.id"; // Pastikan ini domain yang benar
const BASE_URL =
...
if (!API_KEY) {
  console.error(
    "❌ ERROR: API Key tidak ditemukan! Pastikan file .env ada dan library 'dotenv' terinstall.",
  );
    ...
    for (const card of cardsRaw) {
      ...
      if (cachedCard && isTranslated) {
        ...
        groupedData[charName].data.push({
          ...
          hasAwakening:
            card.initialRarity === 5 && card.assetId.includes("link"), // Simplified logic check
      ...
      const formatSkill = (raw, prefix) => {
        ...
        const nameId = aiRes
          ...
          : translateWithRegex(nameJp, "id");
        const nameEn = aiRes
          ? aiRes[`${prefix}_name_en`]
          : translateWithRegex(nameJp, "en");
        const descId = aiRes
          ? aiRes[`${prefix}_desc_id`]
          : translateWithRegex(descJp, "id");
        const descEn = aiRes
          ? aiRes[`${prefix}_desc_en`]
          : translateWithRegex(descJp, "en");
      ...
      const formatYell = (raw) => {
        ...
        const descId = aiRes
          ? aiRes[`yell_desc_id`]
          : translateWithRegex(raw.desc, "id");
        const descEn = aiRes
          ? aiRes[`yell_desc_en`]
        ...
        return {
          ...
          source: {
            initialImage: `${R2_DOMAIN}/iconSkillYell/img_yell_icon_${raw.iconId}.png`,
          },
        };
      };
      ...
      const images = {
        ...
        fullEvolved: hasEvolvedArt
          ? `${R2_CARDS}/cardFull/img_card_full_${evolvedIndex}_${assetId}.webp`
          : null,
        upperEvolved: hasEvolvedArt
          ? `${R2_CARDS}/cardUpper/img_card_upper_${evolvedIndex}_${assetId}.png`
          : null,
